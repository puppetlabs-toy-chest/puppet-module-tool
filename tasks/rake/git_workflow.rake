...
task :git_setup do
	...
	@remote = {}
	default_remote = {}
	default_remote[:url] = 'git://github.com/puppetlabs/puppet-module-tool'
	default_remote[:name] = 'origin'
	@remote[:name] = %x{git config pmtt.defaultremote}.chomp
	...
	@remote[:fetch] = %x{git config pmt.#{@remote[:name]}.fetch}.chomp
...
task :mail_patches do
    ...
    sh "git format-patch -C -M -s -n --subject-prefix='PATCH/puppet-module-tool' #{parent}..HEAD"
