...
#
# Examples:
#   # Read from default "config/secrets.yml" and "config/secrets~sample.yml" files:
...
class SecretsReader
  ...
  #   "config/secret.yml" and "config/secret~sample.yml".
  #
  # Options:
  ...
  def self.read(*args)
    ...
    sample_file = "config/secrets~sample.yml"
    ...
    if object = self.filename_to_ostruct(given_file)
      ...
      message << "WARNING! Using insecure default '#{sample_file}' settings, see README.md"
      error = true
    else
